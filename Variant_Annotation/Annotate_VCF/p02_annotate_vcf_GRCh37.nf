...
process VEP_annotate {
     ...
     script:
     """
     if vep -i ${vcf} --plugin LoF,loftee_path:${loftee},human_ancestor_fa:${loftee}/human_ancestor_fa.gz,conservation_file:${loftee}/phylocsf_gerp.sql --dir_plugins ${loftee} --plugin Carol --plugin Condel,:/opt/.vep/Plugins/config/Condel/config,b --plugin ExACpLI,/opt/gnomad.v2.1.1.oe_lof.by_gene.txt --plugin LoFtool,/opt/.vep/Plugins/LoFtool_scores.txt --plugin CADD,${CADD_SNV},${CADD_INDEL} -o ${sample}.${chrom}.vep.vcf.gz --cache --force_overwrite --buffer_size 10000 --species homo_sapiens --assembly GRCh37 --dir ${vep_db} --offline --fork 1 --hgvs -e --fa ${ref_fa} --minimal --allele_number --check_existing --vcf --compress_output bgzip; then 
